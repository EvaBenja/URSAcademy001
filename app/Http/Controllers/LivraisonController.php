...
class LivraisonController extends Controller
{
    ...
    public function validerCloture(Request $request, $id)
    {
        ...
        if ($livraisonFresh) {
            \App\Http\Controllers\RepartitionController::repartir($livraisonFresh);
        }
    ...
    public function retirerLivreur(Request $request, $id)
    {
        ...
        if (!in_array($livraison->statut, ['validee', 'en_attente'])) {
            return response()->json(['message' => "Impossible — la course est déjà en cours ou terminée"], 422);
        }
        ...
        return response()->json([
            'message'   => 'Livreur retiré — course remise en attente',
            'livraison' => $livraison->load('livreur'),
        ]);
    }
}